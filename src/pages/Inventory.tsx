...
import { Plus, Search, Bed, Lock, Unlock, CheckCircle2, AlertTriangle, Home, Filter, ImagePlus } from 'lucide-react';
...
const Inventory = () => {
  ...
  const updateRoom = useUpdateRoom(); // unused for now — kept for future
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    ...
    if (!form.property_id || !form.room_number.trim()) {
      toast.error('Property and room number are required');
      return;
    }
    ...
    if (form.photoFile) {
      try {
        ...
        toast.error('Photo upload failed — room added without photo');
        console.error(err);
      }
    }
    ...
    try {
      await createRoom.mutateAsync({
        ...
        photo_url: photoUrl,           // assuming your schema supports photo_url
      });
  ...
  const handleConfirm = async (roomId: string) => {
    try {
      ...
      toast.error(err.message || 'Failed to confirm status');
    }
  };
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            ...
            <DialogContent className="sm:max-w-[460px]">
              ...
              <form onSubmit={handleAddRoom} className="space-y-4 mt-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <Label className="text-xs">Expected Rent ₹</Label>
                    <Input
                      type="number"
                      placeholder="8000"
                      value={form.expected_rent}
                      onChange={(e) => setForm((f) => ({ ...f, expected_rent: e.target.value }))}
                    />
                  </div>
                </div>
                ...
                {/* Photo Upload */}
                <div className="space-y-1">
                  <Label className="text-xs">Room Photo (optional)</Label>
                  ...
                  {form.photoFile && (
                    <p className="text-[10px] text-muted-foreground">
                      {form.photoFile.name} • {(form.photoFile.size / 1024).toFixed(1)} KB
                    </p>
                  )}
                </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <Input
              placeholder="Search rooms, properties, owners..."
              value={search}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-xs mt-1">Try adjusting filters or add new rooms</p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((room: any) => {
              ...
              const lastConfirmed = room.last_confirmed_at
                ? formatDistanceToNow(new Date(room.last_confirmed_at), { addSuffix: true })
                : 'Never';
              ...
              return (
                <div
                  ...
                  className={`p-4 rounded-xl border bg-card hover:shadow-md transition-shadow ${
                    room.auto_locked ? 'border-orange-400/50 bg-orange-50/30 dark:bg-orange-950/10' : ''
                  }`}
                >
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span className="text-muted-foreground">Beds:</span> {room.bed_count}
                    </div>
                    {room.room_type && (
                      <div>
                        <span className="text-muted-foreground">Type:</span> {room.room_type}
                      </div>
                    )}
                    {room.expected_rent && (
                      <div>
                        <span className="text-muted-foreground">Ask:</span> ₹
                        {Number(room.expected_rent).toLocaleString()}
                      </div>
                    )}
                    {room.actual_rent && (
                      <div>
                        <span className="text-muted-foreground">Last:</span> ₹
                        {Number(room.actual_rent).toLocaleString()}
                      </div>
                    )}
                    {room.vacating_date && (
                      <div className="col-span-2">
                        <span className="text-muted-foreground">Vacating:</span> {room.vacating_date}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t flex items-center justify-between">
                    ...
                    <Dialog
                      open={confirmOpen === room.id}
                      onOpenChange={(v) => setConfirmOpen(v ? room.id : null)}
                    >
                      <DialogTrigger asChild>
                        ...
                        <div className="space-y-3 mt-2">
                          <div className="space-y-1">
                            ...
                            <Select
                              ...
                              onValueChange={(v) => setConfirmForm((f) => ({ ...f, status: v }))}
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
